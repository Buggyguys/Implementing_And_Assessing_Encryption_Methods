...
// Set the key size for the Camellia context
void camellia_set_key_size(camellia_context_t* context, int key_size) {
    if (!context) return;
    
    // Validate key size (Camellia supports 128, 192, and 256 bits)
    if (key_size != 128 && key_size != 192 && key_size != 256) {
        fprintf(stderr, "Warning: Invalid Camellia key size %d, defaulting to 256 bits\n", key_size);
...
// Set the mode of operation for the Camellia context
void camellia_set_mode(camellia_context_t* context, const char* mode) {
    ...
    if (strcmp(mode_upper, "GCM") == 0 ||
        ...
        fprintf(stderr, "Warning: Invalid Camellia mode %s, defaulting to GCM\n", mode);
        strncpy(context->mode, "GCM", sizeof(context->mode) - 1);
    }
}

// Get the appropriate IV length for the given mode
int camellia_get_iv_length(const char* mode) {
    ...
    // Return the appropriate IV length based on the mode
    if (strcmp(mode_upper, "GCM") == 0) {
