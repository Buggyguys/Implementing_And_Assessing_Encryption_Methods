...
def encrypt_stdlib(data, key, iv=None):
    ...
    # generate IV if not already existing  
    if iv is None:
        iv = get_stdlib_iv("OFB")
    
    # chunk processing if data > max
...
def encrypt_custom(data, key, iv=None):
    ...
    # encrypt IV, then XOR with plaintext
    shift_register = bytearray(iv)
    
    # process data byte by byte (or chunks)
...
def decrypt_custom(ciphertext, key):
    ...
    # OFB mode decryption (same process)
    result = bytearray()
    shift_register = bytearray(iv)
