#!/usr/bin/env python3
"""
CryptoBench Pro - ECC P-521 Implementation
Specialized implementation for the P-521 (secp521r1) curve.
"""
...
from .key_utils import (
    ...
    CURVE_PARAMS, P521, add_chunk_delimiter, split_delimited_chunks
)

class ECCP521Implementation(ECCImplementationBase):
    """Specialized ECC implementation for the P-521 curve."""
    
    def __init__(self, **kwargs):
        """Initialize with P-521 curve."""
        # Remove curve from kwargs if it exists to avoid conflict
        if 'curve' in kwargs:
            kwargs.pop('curve')
        super().__init__(curve="P-521", **kwargs)
        self.name = "ECC-P521"
        self.description = "P-521 (secp521r1)"
        if self.is_custom:
    ...
    def encrypt(self, data, public_key=None):
        """
        Encrypt data using ECIES with P-521 curve.
    ...
    def decrypt(self, ciphertext, private_key=None):
        """
        Decrypt data using ECIES with P-521 curve.
    ...
    def sign(self, data, private_key=None):
        """
        Sign data using ECDSA with P-521 curve.
    ...
    def verify(self, data, signature, public_key=None):
        """
        Verify signature using ECDSA with P-521 curve.
    ...
    def _encrypt_stdlib(self, data, public_key):
        """Encrypt data using the standard library implementation of ECIES with P-521."""
        # Use the P-521 curve
        curve = ec.SECP521R1()
        ...
        # Derive encryption key from shared secret - use SHA512 for P-521
        derived_key = HKDF(
    ...
    def _decrypt_stdlib(self, ciphertext, private_key):
        """Decrypt data using the standard library implementation of ECIES with P-521."""
        ...
        try:
            ...
            # Derive the encryption key - use SHA512 for P-521
            derived_key = HKDF(
                algorithm=hashes.SHA512(),
                length=32,
                salt=None,
    ...
    def _sign_stdlib(self, data, private_key):
        """Sign data using the standard library implementation of ECDSA with P-521."""
        # Hash the data with SHA512 (matching curve size)
    ...
    def _verify_stdlib(self, data, signature, public_key):
        """Verify signature using the standard library implementation of ECDSA with P-521."""
        # Hash the data with SHA512 (matching curve size)
        data_hash = hashlib.sha512(data).digest()
    ...
    def _encrypt_custom(self, data, public_key):
        """Encrypt data using a custom implementation of ECIES with P-521 curve."""
        # Use the P-521 curve parameters
        curve = P521
        ...
        # Derive encryption key using SHA512 (matching curve size)
        key_material = hmac.new(b'ECIES', shared_secret, hashlib.sha512).digest()
        encryption_key = key_material[:32]  # Use a stronger key for P-521
        ...
        # Compute MAC over IV and ciphertext using SHA512
        mac = hmac.new(mac_key, iv + ciphertext, hashlib.sha512).digest()
    ...
    def _decrypt_custom(self, ciphertext, private_key):
        """Decrypt data using a custom implementation of ECIES with P-521 curve."""
        try:
            ...
            # Verify MAC
            computed_mac = hmac.new(mac_key, iv + actual_ciphertext, hashlib.sha512).digest()
    ...
    def _sign_custom(self, data, private_key):
        """Sign data using a custom implementation of ECDSA with P-521 curve."""
        # Use P-521 curve parameters
        curve = P521
        ...
        h1 = hashlib.sha512(data_hash + d.to_bytes((curve["bits"] + 7) // 8, byteorder='big')).digest()
    ...
    def _verify_custom(self, data, signature, public_key):
        """Verify signature using a custom implementation of ECDSA with P-521 curve."""
        # Use P-521 curve parameters
        curve = P521
        coord_size = (curve["bits"] + 7) // 8
