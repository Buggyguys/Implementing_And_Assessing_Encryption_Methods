#!/usr/bin/env python3
"""
CryptoBench Pro - AES-OFB Implementation
Provides AES encryption/decryption in OFB mode.
"""
...
def encrypt_stdlib(data, key, iv=None):
    """
    Encrypt data using PyCryptodome AES-OFB implementation.
    ...
    if len(data) > MAX_INPUT_SIZE:
        ...
        for i in range(0, len(data), MAX_INPUT_SIZE):
            ...
            cipher = CryptoAES.new(key, CryptoAES.MODE_OFB, iv)
...
def decrypt_stdlib(ciphertext, key):
    """
    Decrypt ciphertext using PyCryptodome AES-OFB implementation.
    ...
    # Decrypt the data (OFB decryption is the same as encryption)
    cipher = CryptoAES.new(key, CryptoAES.MODE_OFB, iv)
    return cipher.decrypt(actual_ciphertext)

def encrypt_custom(data, key, iv=None):
    """
    Encrypt data using custom AES-OFB implementation.
    ...
    # OFB mode: encrypt IV, then XOR with plaintext
    ...
    for i in range(0, len(data)):
        ...
        # Update shift register: shift left and add encrypted byte (not cipher byte like CFB)
        shift_register = shift_register[1:] + bytearray([encrypted_sr[0]])
    
    return bytes(result)

def decrypt_custom(ciphertext, key):
    """
    Decrypt ciphertext using custom AES-OFB implementation.
    ...
    for i in range(len(actual_ciphertext)):
        ...
        # Update shift register: shift left and add encrypted byte (not cipher byte)
        shift_register = shift_register[1:] + bytearray([encrypted_sr[0]])
